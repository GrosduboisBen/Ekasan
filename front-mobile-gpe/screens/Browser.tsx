...
const Browser = ({ route }: IBrowserProps) => {
  ...
  const moveSelectedFiles = async (destination: string) => {
    ...
    try {
      const dirInfo = await FileSystem.getInfoAsync(destination + '/folder');
      if (!dirInfo.exists) {
        await FileSystem.makeDirectoryAsync(destination + '/folder', { intermediates: true });
      }
    } catch (error) {
    ...
    const destinationFolderFiles = await FileSystem.readDirectoryAsync(destination + '/folder');
    
    function executeTransfer() {
      const transferPromises = selectedFiles.map((file) => {
        const destPath = `${destination}/folder/${file.name}`;
        return moveOrCopy === 'Copy'
          ? FileSystem.copyAsync({ from: `${currentDir}/folder/${file.name}`, to: destPath })
          : FileSystem.moveAsync({ from: `${currentDir}/folder/${file.name}`, to: destPath });
      });
    ...
    const conflictingFiles = selectedFiles.filter((file) => destinationFolderFiles.includes(file.name));
    const confLen = conflictingFiles.length;
    if (confLen > 0) {
      Alert.alert(
        'Conflicting Files',
        `The destination folder has ${confLen} ${confLen === 1 ? 'file' : 'files'} with the same ${confLen === 1 ? 'name' : 'names'}.`,
        [
  ...
  const onRename = async () => {
    try {
      ...
      const fileFolderPath = filePathSplit
        ...
        .join('/folder/');
      const newFilePath = `${fileFolderPath}/folder/${newFileName}`;
      ...
      console.error('Error renaming the file/folder:', error);
      handleSetSnack({
