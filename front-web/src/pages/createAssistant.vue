...
const promptInstruction = ref(undefined as undefined | string)
const assistantDescription = ref('')

const selectedStore = ref(null as null | string)

const clearForm = () => {
  assistantName.value = ''
  promptInstruction.value = undefined
  assistantDescription.value = ''
}

const createAssistant = async (event: any) => {
  const newAssistant = {
...
<template>
  <VCard v-if="!loadingStatus">
    <VCardTitle>
      Create you assistant
    </VCardTitle>
    <VForm
      ref="refForm"
      @submit.prevent="createAssistant"
    >
      <VCardItem>
        <AppTextField
          id="name"
